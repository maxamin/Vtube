...
def draw_face_hull_mask(image,image_landmarks, eyebrows_expand_mod=1.0):
    ...
    array_parts=[*r_jaw, *l_jaw, *r_cheek, *l_cheek, *nose_ridge, *r_eye, *l_eye, *nose]
    array_parts=np.concatenate(array_parts)
    hull=cv2.convexHull(array_parts)
    cv2.polylines(image,hull,True,(0,0,255),thickness=5)
...
def estimate_pitch_yaw_roll_by_degree(aligned_landmarks, size=256):
    pitch, yaw, roll=estimate_pitch_yaw_roll(aligned_landmarks)
    pitch, yaw=-pitch, -yaw
    angle_per_pi=180/3.14;
    return round(-pitch*angle_per_pi), round(yaw*angle_per_pi), round(roll*angle_per_pi)
