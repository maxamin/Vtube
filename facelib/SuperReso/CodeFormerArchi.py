...
class VectorQuantizer(nn.Module):
    ...
    def get_codebook_feat(self, indices, shape):
        ...
        # get quantized latent vectors
        z_q = torch.matmul(min_encodings.half(), self.embedding.weight)
...
model=None;
def process_img(img,w=0.5,device="cuda"):
    global model;
    ...
    tv_normalize(img_tensor, (0.5, 0.5, 0.5), (0.5, 0.5, 0.5), inplace=True)
    input=img_tensor.unsqueeze(0).to(device)
    with torch.no_grad():
        start_time_tick=time.time()
        output_tensor = model(input, w=0.5, adain=True)[0]
        end_time_tick=time.time()
        print(f"模型推理耗时:{end_time_tick-start_time_tick}秒")
        img=output_tensor.squeeze(0).float().detach().cpu().numpy()
