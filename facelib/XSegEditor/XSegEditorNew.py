...
from core.localization import StringsDB, system_language
...
class QCanvas(QFrame):
    def __init__(self):
        ...
        self.operate_canvas = QCanvasOperator(cbar)
        self.ui_layout = QHBoxLayout()
        self.ui_layout.setContentsMargins(0,0,0,0)
        self.ui_layout.addWidget(self.canvas_control_left_bar)
        self.ui_layout.addWidget(self.operate_canvas)
        self.ui_layout.addWidget(self.canvas_control_right_bar)
...
class MainWindow(QXMainWindow):
    ...
    def on_loader_finish(self, image_paths, image_paths_has_ie_polys):
        ...
        self.onclick_next_image(first_initialization=True)
    ...
    def canvas_initialize(self, image_path, only_has_polys=False):
        ...
        if img is None:
            return False
        self.canvas.operate_canvas.initialize ( img,  ie_polys=ie_polys, xseg_mask=xseg_mask )
        #self.folder_label.setText(f"{self.input_dirpath}")
        self.filename_label.setText(f"{self.input_dirpath}\\{image_path.name}")
        return True

    def canvas_finalize(self, image_path,convert_mask=False):
        self.canvas.operate_canvas.finalize()

        if image_path.exists():
            dflimg = DFLIMG.load(image_path)
            ie_polys = dflimg.get_seg_ie_polys()
            new_ie_polys = self.canvas.operate_canvas.get_ie_polys()
            ...
            if self.autosave_checkbox.isChecked() or convert_mask:
                if new_ie_polys.has_polys():
                    self.apply_canvas_polygon_to_mask(dflimg,new_ie_polys)
            dflimg.save()

        self.filename_label.setText(f"")

    def apply_canvas_polygon_to_mask(self,dflimg,seg_ie_polys,reso=256):

        mask=np.zeros_like(dflimg.get_img()).astype(float)
        if seg_ie_polys is not None:
            if seg_ie_polys.has_polys():
                seg_ie_polys.overlay_mask(mask)
        mask=cv2.resize(mask,(reso,reso))
    ...
    def onclick_prev_image(self):
        ...
        if self.canvas.operate_canvas.is_initialized():
    ...
    def onclick_next_image(self, first_initialization=False):
        key_mods = QApplication.keyboardModifiers()
    ...
    def onclick_from_landmark_points(self):
        self.showNotify("从特征点提取编辑多边形")
        image_path=self.image_paths[0]
        #print(image_path)
        if image_path.exists():
            dflimg = DFLIMG.load(image_path)
            dflimg.create_seg_polygon_from_landmarks()
        self.canvas_initialize(self.image_paths[0])

    def onclick_from_xseg_mask(self):        
        image_path=self.image_paths[0]
        if image_path.exists():
            dflimg = DFLIMG.load(image_path)
            dflimg.create_seg_polygon_from_mask()
        self.canvas_initialize(self.image_paths[0])
        self.showNotify("已经从XSeg提取编辑多边形")

    def onclick_clear_polygons(self):        
        image_path=self.image_paths[0]
        if image_path.exists():
            dflimg = DFLIMG.load(image_path)
            dflimg.clear_ie_polygon()
        self.canvas_initialize(self.image_paths[0])
        self.showNotify("清空编辑多边形")

    def onclick_mouth_mask(self):
        image_path=self.image_paths[0]
        if image_path.exists():
            dflimg = DFLIMG.load(image_path)
            dflimg.create_mouth_ie_polygon()
        self.canvas_initialize(self.image_paths[0])
        self.showNotify("已经添加嘴巴区域可编辑多边形")

    def onclick_teeth_mask(self):        
        image_path=self.image_paths[0]
        if image_path.exists():
            dflimg = DFLIMG.load(image_path)
            dflimg.create_teeth_ie_polygon()
        self.canvas_initialize(self.image_paths[0])
        self.showNotify("已经添加口内牙齿区域可编辑多边形")

    def onclick_eyes_mask(self):
        self.showNotify("提取眼睛区域")
        image_path=self.image_paths[0]
        if image_path.exists():
            dflimg = DFLIMG.load(image_path)
            dflimg.create_eyes_ie_polygon()
        self.canvas_initialize(self.image_paths[0])

    def onclick_convert_xseg_mask(self):
        image_path=self.image_paths[0]
        if image_path.exists():
            dflimg = DFLIMG.load(image_path)
            self.apply_canvas_polygon_to_mask(dflimg, self.canvas.operate_canvas.get_ie_polys())
            self.canvas_finalize(image_path,convert_mask=True)
            self.showNotify("已经转换为Xseg遮罩")

    def onclick_save_edit(self):
        image_path=self.image_paths[0]
        if image_path.exists():
            if self.canvas.operate_canvas.is_initialized():
                self.canvas_finalize(self.image_paths[0],convert_mask=True)
            self.canvas_initialize(self.image_paths[0], False)
            self.showNotify("保存修改")

    def showNotify(self,str):
        timestr=time.strftime("%H:%M:%S",time.localtime())
        self.tips_label.setText(f"[{timestr}]{str}")
    #-----设置界面
    def initialize_ui(self):
        ...
        btn_from_landmark_points = QXIconButton(QIconDB.landmark,"create editable polygons from landmarks points \n(从人脸特征点创建可绘制多边形)", shortcut='L', click_func=self.onclick_from_landmark_points)
        btn_from_landmark_points.setIconSize(QUIConfig.icon_q_size)

        btn_from_xseg_mask = QXIconButton(QIconDB.xseg_mask, "create editable polygons from xseg mask \n(从遮罩创建可绘制多边形)", shortcut='M', click_func=self.onclick_from_xseg_mask)
        btn_from_xseg_mask.setIconSize(QUIConfig.icon_q_size)

        btn_clear_polygons = QXIconButton(QIconDB.clear, "clear all editable polygons\n(清除全部可绘制多边形)", shortcut='C', click_func=self.onclick_clear_polygons)
        btn_clear_polygons.setIconSize(QUIConfig.icon_q_size)

        btn_mouth_mask = QXIconButton(QIconDB.mouth, "extract mouth area polygon\n(提取嘴部区域多边形)", shortcut='P', click_func=self.onclick_mouth_mask)
        btn_mouth_mask.setIconSize(QUIConfig.icon_q_size)

        btn_eyes_area = QXIconButton(QIconDB.eyes, "extract eyes area polygon\n(提取眼睛区域作为多边形)", shortcut='E', click_func=self.onclick_eyes_mask)
        btn_eyes_area.setIconSize(QUIConfig.icon_q_size)

        btn_teeth_mask = QXIconButton(QIconDB.teeth, "get teeth(inner mouth) area polygon \n(提取牙齿区域作为多边形)", shortcut='T', click_func=self.onclick_teeth_mask)
        btn_teeth_mask.setIconSize(QUIConfig.icon_q_size)

        btn_calc_mask_image = QXIconButton(QIconDB.ok, "计算遮罩\ncalculate mask from editable polygon", shortcut='V', click_func=self.onclick_convert_xseg_mask)
        btn_calc_mask_image.setIconSize(QUIConfig.icon_q_size)  

        btn_save_image = QXIconButton(QIconDB.save, "save result(保存结果)", shortcut='S', click_func=self.onclick_save_edit)
        btn_save_image.setIconSize(QUIConfig.icon_q_size)     

      

        #--- 执行图片操作按钮块
        image_operate_bar_layout = QHBoxLayout()
        image_operate_bar_layout.setContentsMargins(0,0,0,0)
           
        image_operate_bar_layout.addWidget ( btn_eyes_area, alignment=Qt.AlignCenter)
        image_operate_bar_layout.addWidget ( btn_mouth_mask, alignment=Qt.AlignCenter)
        image_operate_bar_layout.addWidget ( btn_teeth_mask, alignment=Qt.AlignCenter) 
        image_operate_bar_layout.addWidget ( btn_from_landmark_points, alignment=Qt.AlignCenter)
        image_operate_bar_layout.addWidget ( btn_from_xseg_mask, alignment=Qt.AlignCenter)     
        image_operate_bar_layout.addWidget ( btn_teeth_mask, alignment=Qt.AlignCenter)
        image_operate_bar_layout.addWidget ( btn_teeth_mask, alignment=Qt.AlignCenter)

        image_operate_bar_layout.addWidget ( btn_clear_polygons, alignment=Qt.AlignCenter)
        #image_operate_bar_layout.addWidget ( btn_calc_mask_image, alignment=Qt.AlignCenter)
        image_operate_bar_layout.addWidget ( btn_save_image, alignment=Qt.AlignCenter)    

        image_operate_bar_frame = QFrame()
        image_operate_bar_frame.setSizePolicy ( QSizePolicy.Fixed, QSizePolicy.Fixed )
        image_operate_bar_frame.setLayout(image_operate_bar_layout)
        
        #--图片预览前后导航块
        preview_image_bar_layout_l = QHBoxLayout()
        preview_image_bar_layout_l.setContentsMargins(0,0,0,0)
        ...
        btn_delete_image.setIconSize(QUIConfig.icon_q_size)

        btn_prev_image = QXIconButton(QIconDB.left, QStringDB.btn_prev_image_tip, shortcut='A', click_func=self.onclick_prev_image)
        btn_prev_image.setIconSize(QUIConfig.preview_bar_icon_q_size)

        btn_next_image = QXIconButton(QIconDB.right, QStringDB.btn_next_image_tip, shortcut='D', click_func=self.onclick_next_image)
        btn_next_image.setIconSize(QUIConfig.preview_bar_icon_q_size)

        preview_image_bar_layout_l.addWidget ( btn_prev_image, alignment=Qt.AlignCenter)
        preview_image_bar_layout_l.addWidget ( image_bar)
        preview_image_bar_layout_l.addWidget ( btn_next_image, alignment=Qt.AlignCenter)
        preview_image_bar_layout_l.addWidget ( btn_delete_image, alignment=Qt.AlignCenter)
        ...
        preview_image_bar.setLayout(preview_image_bar_layout_l)
        ...
        self.folder_label.setFont(label_font)
        self.has_ie_polys_count_label = QLabel()

        #==信息栏（文件夹名、文件名、标记数）
        info_frame_layout = QHBoxLayout()
        info_frame_layout.setContentsMargins(0,0,0,0)
        info_frame_layout.addWidget ( self.folder_label, alignment=Qt.AlignCenter)
        info_frame_layout.addWidget (self.filename_label, alignment=Qt.AlignCenter)
        info_frame_layout.addWidget (self.has_ie_polys_count_label, alignment=Qt.AlignCenter)
        info_frame = QFrame()
        ...
        self.autosave_checkbox.setText(QStringDB.AutoConvertMaskText)
        self.autosave_checkbox.setChecked(False)
        status_frame_layout = QHBoxLayout()
        status_frame_layout.addWidget ( self.autosave_checkbox, alignment=Qt.AlignLeft)
        status_frame_layout.addWidget ( self.tips_label, alignment=Qt.AlignLeft)
        status_frame = QFrame()
        ...
        main_canvas_layout.addWidget (image_operate_bar_frame)
        main_canvas_layout.addWidget (info_frame)
        main_canvas_layout.addWidget (self.canvas)      
        main_canvas_layout.addWidget (status_frame)
        main_canvas_layout.addWidget (preview_image_bar)

        self.main_canvas_frame = QFrame()
        self.main_canvas_frame.setLayout(main_canvas_layout)
