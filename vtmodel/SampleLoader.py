...
import core.samplelib.PackedFaceset
from core import pathex
from core.interact import interact as io
from core.joblib import Subprocessor
from core.DFLIMG import *
from facelib import FaceType, LandmarksProcessor
import core.samplelib as samplelib
from vtmodel.Sample import Sample, SampleType
...
class SampleLoader:
    ...
    def load(sample_type, samples_path, subdirs=False):
        ...
        if            sample_type == SampleType.IMAGE:
            if  samples[sample_type] is None:
                samples[sample_type] = [ Sample(filename=filename) for filename in io.progress_bar_generator( pathex.get_image_paths(samples_path, subdirs=subdirs), "[S]Loading") ]
    ...
    @staticmethod
    def load_face_samples ( image_paths,sample_name="素材"):
        result = FaceSamplesLoaderSubprocessor(image_paths,sample_name).run()
        sample_list = []

        for filename, data in result:
            if data is None:
                continue
            face_type,shape,landmarks,seg_ie_polys,xseg_mask_compressed,eyebrows_expand_mod,source_filename  = data
            sample=Sample(filename=filename,sample_type=SampleType.FACE,face_type=FaceType.fromString (face_type),
                            shape=shape,landmarks=landmarks,seg_ie_polys=seg_ie_polys,
                            xseg_mask_compressed=xseg_mask_compressed,eyebrows_expand_mod=eyebrows_expand_mod,
...
class FaceSamplesLoaderSubprocessor(Subprocessor):
    #override
    def __init__(self, image_paths,sample_name="素材"):
    ...
    def on_clients_initialized(self):
        name=os.path.basename(self.sample_name)
        io.progress_bar (f"[S]加载素材样本({name})", len (self.image_paths))
