...
def get_file_paths_str(dir_path,subdir=True):
    dir_path = Path (dir_path)

    if dir_path.exists():
        if subdir is True:
            return [x for x in sorted([ x.name for x in list(scandir(str(dir_path))) if x.is_file() ]) ]
        else:
            return [file for file in listdir(dir_path) if "." in file]
    else:
        return []
...
def get_first_file_contain_word (dir_path, name,fullpath=True):
    files=sorted([ x.name for x in list(scandir(str(dir_path))) if x.is_file() ])
    for file in files:
        if name in file:
            if fullpath:
...
def get_first_file_end_with (dir_path, word,exclude="*",fullpath=True):
    files=sorted([ x.name for x in list(scandir(str(dir_path))) if x.is_file() ])
    for file in files:
        if exclude in file:
            continue;
