...
from core.swapmodels import ModelBase
from core.samplelib import SampleProcessor
from core.samplelib import SampleGeneratorFace
#from vtmodel.FaceSampleGenerator import FaceSampleGenerator as SampleGeneratorFace
class SAEHDModel(ModelBase):
    ...
    def on_initialize_options(self):
        ...
        if self.is_first_run():
            ...
            while True:
                archi = io.input_str ("AE architecture", default_archi, help_message="Examples: df, liae, df-d, df-ud, liae-ud").lower()
    ...
    def on_initialize(self):
        #print("SAEHD model on_initialize function start")
        device_config = nn.getCurrentDeviceConfig()
        ...
        #    use_fp16 = io.input_bool ("Export quantized use fp16 ?", False, help_message='Makes the exported model faster. If you have problems, disable this option.')
        ...
        #print(f" Create DeepFakeArchi{resolution}:{archi_type}-{archi_opts}")
    ...
    def export_dfm (self,save_dir=""):
        import os;
        if os.path.exists(save_dir) is False:
            print("导出模型目录不存在");
            return

        output_path=f"{save_dir}/{self.get_model_name()}.dfm".replace('_SAEHD','');
        io.log_info(f'开始导出模型到:{output_path}')
    ...
    #override
    def onSave(self):
        for model, filename in io.progress_bar_generator(self.get_model_filename_list(), "[T]Saving", leave=False):
    ...
    def onGetPreview(self, samples, for_history=False):
        ...
        n_samples = min(4, self.get_batch_size(), 1000 // self.resolution )
        ...
        if self.resolution <= 320:
            ...
            st_m = []
            for i in range(n_samples):
                ar = S[i]*target_srcm[i], SS[i]*target_srcm[i]
                st_m.append ( np.concatenate ( ar, axis=1) )
    ...
    def load_test_sample_folder(self,test_sample_folder,batch_size=6,face_type=FaceType.WHOLE_FACE, random_warp=True,ct_mode=None):
        resolution=self.options.get("resolution",256)
        print("\n resolution:",resolution)
        #test_sample_folder=Path(test_sample_folder)
        try:
            self.test_data_generator=SampleGeneratorFace(test_sample_folder, random_ct_samples_path=None, debug=False, batch_size=batch_size,
                        sample_process_options=SampleProcessor.Options(scale_range=[-0.15, 0.15], random_flip=True),
                        output_sample_types = [ {'sample_type': SampleProcessor.SampleType.FACE_IMAGE,'warp':random_warp, 'transform':True,'channel_type' : SampleProcessor.ChannelType.BGR, 'ct_mode': ct_mode, 'random_hsv_shift_amount' : False,  'face_type':face_type, 'data_format':nn.data_format, 'resolution': resolution},
            ...
            print(f"finish load_test_sample_folder {test_sample_folder}")
        except Exception as ex:
            print("load test sample error")
            print("出现如下异常%s"%ex)
    
    def get_test_preview(self):
        if self.test_data_generator is None:
            print("!!! Error: test_data_generator is not set !!!")
            return False,None;
        if self.test_data_generator.initialized is False:
            print("!!! Error: test_data_generator is not initialized yet !!!")
            return False,None;

        
        train_samples=self.generate_next_samples()
        test_samples=self.test_data_generator.generate_next()
        #print("test_samples length:",len(test_samples))
        test_preview_samples=[]
        test_preview_samples.append(train_samples[0])
        test_preview_samples.append(test_samples)
        #test_preview_samples.append(train_samples[1])

        previews=self.onGetPreview(test_preview_samples)
        return True,previews

Model_Instance = SAEHDModel
