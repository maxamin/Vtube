...
from core import joblib
from core.interact import interact as io
from core.joblib import SubprocessGenerator, ThisThreadGenerator
from facelib import LandmarksProcessor
from core.samplelib import (SampleGeneratorBase, SampleLoader, SampleProcessor,
                       SampleType)
...
class SampleGeneratorFace(SampleGeneratorBase):
    def __init__ (self, samples_path, debug=False, batch_size=1,
        ...
        if self.debug:
            ...
            self.generators = [SubprocessGenerator ( self.batch_func, (samples, index_host.create_cli(), ct_samples, ct_index_host.create_cli()
                            if ct_index_host is not None else None), start_now=False )  for i in range(self.generators_count) ]
