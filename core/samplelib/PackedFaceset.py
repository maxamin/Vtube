...
import core.samplelib.SampleLoader
from core.interact import interact as io
from core.samplelib import Sample
...
class PackedFaceset():
    ...
    def pack(samples_path):
        ...
        samples = core.samplelib.SampleLoader.load_face_samples(image_paths)
        samples_len = len(samples)

        samples_configs = []
        for sample in io.progress_bar_generator (samples, "[P]Processing"):
        ...
        for sample in io.progress_bar_generator(samples, "[P]Packing"):
            try:
        ...
        for filename in io.progress_bar_generator(image_paths, "[P]Deleting files"):
            Path(filename).unlink()

        if as_person_faceset:
            for dir_name in io.progress_bar_generator(dir_names, "[P]Deleting dirs"):
    ...
    @staticmethod
    def load(samples_path,isPakFile=False):
        
        samples_dat_path = samples_path
        if isPakFile:
            samples_dat_path=samples_path;
        if not samples_dat_path.exists():
            return None
