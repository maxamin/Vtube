...
class InteractBase(object):
    ...
    progress_total=1;progress_idx=0;
    ...
    def progress_bar_inc(self, c):
        ...
        self.progress_idx=self.progress_idx+c;
        percent=round(self.progress_idx*30//self.progress_total)
        f="="*percent;
        print(f"[P]{f}>({self.progress_idx}/{self.progress_total})",end='\r')
    ...
    def progress_bar_generator(self, data, desc=None, leave=True, initial=0):
        ...
        print(desc)
        for x in data:
            percent=round(idx*30//length)
            f="="*percent;
            print(f"[P]{f}({idx}/{length})",end='\r')
            idx=idx+1;
            yield x;
        print("")
